...
The following code is available to do inside a loop for all the light data files
...
lightfile <- if_else(!is.na(info$File.adjlux),
                     info$File.adjlux, info$File.lux)
...
for (step in 1:3) {
  ...
  twl_error <- bind_cols(geoID = geoID, Producer = Producer, 
                         Year = year, Colony = col.name,
                         n_days = n_days,
                         zenith = calib_SGAT[1], 
