...
lat_smooth1<-df$Lat.mean
for (i in 2:(length(df$Lat.mean))){
  ifelse(df$type[i]!=df$type[i-1] & (difftime(df$tFirst[i],df$tFirst[i-1],units = "hours")<30)
         
         ,lat_smooth1[i]<-(df$Lat.mean[i]+df$Lat.mean[i-1])/2,lat_smooth1[i]<-df$Lat.mean[i]) 
}

lon_smooth1<-df$Lon.mean
for (i in 2:(length(df$Lon.mean))){
  ifelse(df$type[i]!=df$type[i-1] & (difftime(df$tFirst[i],df$tFirst[i-1],units = "hours")<30)          
         ,lon_smooth1[i]<-(df$Lon.mean[i]+df$Lon.mean[i-1])/2,lon_smooth1[i]<-df$Lon.mean[i]) 
}  
...
for (i in 2:(length(df$Lon.mean))){
  ifelse(abs(df$Lon.mean[i]-df$Lon.mean[i-1])>300 & df$type[i]!=df$type[i-1] & difftime(df$tFirst[i],df$tFirst[i-1],units = "hours")<30
