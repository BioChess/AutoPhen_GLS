twilight_cleanup <- function(df,breedingloc_lon,breedingloc_lat,months_breeding,months_extensive_legtucking,show_plot){
  require('roll')
  require('baytrends')
  require('chron')
  #datetime_conversion
  ...
  fillMisspred1$predict[month(fillMisspred1$Date)%in%months_nonbreeding]<-fillMissing(fillMisspred1$predict[month(fillMisspred1$Date)%in%months_nonbreeding], span = 10, max.fill = 20)
  fillMisspred2$predict[month(fillMisspred2$Date)%in%months_nonbreeding]<-fillMissing(fillMisspred2$predict[month(fillMisspred2$Date)%in%months_nonbreeding], span = 10, max.fill = 20)
